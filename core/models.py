...
class Faculty (models.Model):
    ...
    faculty_building_id = models.ForeignKey(
...
class MemberInfo (models.Model):
    memberinfo_user = models.OneToOneField(
        Member, on_delete=models.CASCADE, default=None, null=True, verbose_name='Пользователь')
    memberinfo_surname = models.CharField(
        max_length=500, verbose_name='Фамилия')
    ...
    memberinfo_gradee = models.CharField(
        max_length=500, choices=STEPENI, verbose_name='Ученое степень', default='NO')
    memberinfo_gradee_name = models.CharField(
        max_length=500, verbose_name='Ученое звание', choices=ZVANIE, default='NO')
...
class Conference (models.Model):
    conference_org_id = models.ForeignKey(
        Member, on_delete=models.PROTECT, verbose_name='Организатор')
    conference_name = models.CharField(
        max_length=200, verbose_name='Краткое название',)
    conference_full_name = models.CharField(
        max_length=500, verbose_name='Полное название',)
    conference_format_id = models.CharField(
        max_length=500, choices=FORMAT, verbose_name='Формат проведения конференции')
    conference_date_start = models.DateField(
        verbose_name='Дата начала конференции')
    conference_date_end = models.DateField(
        verbose_name='Дата окончания конференции')
    conference_date_reg_start = models.DateField(
        verbose_name='Дата начала регистрации')
    conference_date_reg_end = models.DateField(
        verbose_name='Дата окончания регистрации')
    conference_discription = models.TextField(verbose_name='Описание')
    conference_faculty_id = models.ForeignKey(
...
class ConferenceSections (models.Model):
    conference_sections_conference_id = models.ForeignKey(
        Conference, on_delete=models.PROTECT, verbose_name='Конференция')
    conference_sections_name = models.CharField(
        max_length=50, verbose_name='Название секции')
...
class MemberApplication (models.Model):
    member = models.ForeignKey(
        Member, on_delete=models.PROTECT, related_name='member', verbose_name='Участник')
    conference_id = models.ForeignKey(
        Conference, on_delete=models.PROTECT, verbose_name='Конференция')
    speech_file = models.FileField(verbose_name='Файл', blank=True)
    app_status = models.CharField(max_length=100, choices=Member_STATUS,
                                  verbose_name='Статус рассмотрения', default='Consideration')
    expert_app_comment = models.TextField(
        verbose_name='Комментарий', blank=True)
    participation_form = models.CharField(
        max_length=100, choices=FORMAT, verbose_name='Формат участия в конференции')
    member_section = models.ForeignKey(
        ConferenceSections, on_delete=models.PROTECT, verbose_name='Секция')
    expert = models.ForeignKey(Member, on_delete=models.PROTECT,
                               related_name='expert', verbose_name='Эксперт', null=True, blank=True)
    co_authors = models.TextField(max_length=500, verbose_name='Соавторы')
    speech_name = models.CharField(
        max_length=100, verbose_name='Название темы')
    speech_keywords = models.CharField(
        max_length=100, verbose_name='Ключевые слова')
    speech_annotation = models.TextField(
        max_length=1000, verbose_name='Аннотация')
    hotel_required = models.BooleanField(verbose_name='Гостиница', blank=True)
    invitation_required = models.BooleanField(
        verbose_name='Приглашение', blank=True)
...
class ExpertKeywords(models.Model):
    expert = models.OneToOneField(
        Member, on_delete=models.PROTECT, verbose_name='Эксперт')
    keywords = models.TextField(
        verbose_name='Ключевые слова эксперта', blank=True)
...
class ExpertArticle(models.Model):
    expert = models.ForeignKey(
        Member, on_delete=models.PROTECT, null=True, verbose_name='Эксперт')
    article = models.FileField(verbose_name='Docx файл со статьей', blank=True)
    article_keywords = models.TextField(
        verbose_name='Ключевые слова статьи', null=True, blank=True)
